...
Процедура ТестДолжен_ПроверитьЧтоБазаЯвляетсяБазой1С() Экспорт
	...
	ЭтоБаза1С = СтруктураХраненияИБ.ЭтоБаза1С();
...
&Тест
Процедура ТестДолжен_ПолучитьСписокБаз() Экспорт
	...
	СписокБаз = РаботаССУБД.СписокБаз();

	ТекстОшибки = СтрШаблон("Ошибка получения списка баз на сервере ""%1""", ПодключениеКСУБД.Сервер());

	Утверждения.ПроверитьРавенство(СписокБаз[ИмяБД].Имя, ИмяБД, ТекстОшибки);

КонецПроцедуры // ТестДолжен_ПолучитьСписокБаз()
...
Процедура ТестДолжен_ПроверитьСуществованиеТаблицыВБазе() Экспорт
	...
	ТаблицаСуществует = РаботаССУБД.ТаблицаСуществует(ИмяБД, ИмяТаблицы);

	ТекстОшибки = СтрШаблон("Ошибка проверки существования таблицы ""%1"" в базе ""%2/%3""",
	                        ИмяТаблицы,
	                        ПодключениеКСУБД.Сервер(),
	                        ИмяБД);

	Утверждения.ПроверитьИстину(ТаблицаСуществует, ТекстОшибки);

КонецПроцедуры // ТестДолжен_ПроверитьСуществованиеТаблицыВБазе()
...
Процедура ТестДолжен_ПолучитьПоказателиИспользованияТаблицБазы() Экспорт
	...
	ТекстОшибки = СтрШаблон("Ошибка получения показателей использования таблиц базы ""%1""", ИмяБД);

	Утверждения.ПроверитьРавенство(ТаблицыБазы.Количество(), 48, ТекстОшибки);

	ТекстОшибки = СтрШаблон("Ошибка получения показателей использования таблиц базы ""%1"",
	                        | не найдена таблица ""%2""",
	                        ИмяБД,
	                        ТестоваяТаблица);
...
&Тест
Процедура ТестДолжен_ПолучитьПоказателиИспользованияТаблицБазыСоСтатистикойОпераций() Экспорт

	Лог.Информация("Тест: Получение показателей использования таблиц базы данных со статистикой операций");
	...
	ТаблицыБазы = СтруктураХраненияИБ.ПоказателиИспользованияТаблицБазы(, Истина);

	ТестоваяТаблица = "Config";

	ТекстОшибки = СтрШаблон("Ошибка получения показателей использования таблиц базы ""%1""
	                        | со статистикой операций",
	                        ИмяБД);
	...
	Утверждения.ПроверитьНеРавенство(ТекТаблица.ОперацийЧтения, Неопределено, ТекстОшибки);

КонецПроцедуры // ТестДолжен_ПолучитьПоказателиИспользованияТаблицБазыСоСтатистикойОпераций()
