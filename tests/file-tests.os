...
Процедура ТестДолжен_СоздатьПапкуНаSFTP() Экспорт
	...
	АдресСервера       = ПолучитьПеременнуюСреды("SFTP_ADDRESS");
	ПортСервера        = 22;
	ПользовательИмя    = ПолучитьПеременнуюСреды("SFTP_USER_NAME");
	ПользовательПароль = ПолучитьПеременнуюСреды("SFTP_USER_PWD");

	Соединение = Новый КлиентSSH(АдресСервера, ПортСервера, ПользовательИмя, ПользовательПароль);

	ИмяКаталога = "testFolder1";

	РаботаСФайлами.СоздатьПапкуНаSFTP(Соединение, ИмяКаталога);
...
Процедура ТестДолжен_ОтправитьФайлНаSFTP() Экспорт
	...
	РаботаСФайлами.ОтправитьФайлНаSFTP(Соединение, ТестовыйФайл.ПолноеИмя, ИмяКаталога);
	...
	Утверждения.ПроверитьИстину(ФайлыНаСервере.Существует(ИмяКаталога + "/" + ТестовыйФайл.Имя), ТекстОшибки);

	ФайлыНаСервере.УдалитьФайл(ИмяКаталога + "/" + ТестовыйФайл.Имя);
...
Процедура ТестДолжен_ПолучитьФайлСSFTP() Экспорт
	...
	РаботаСФайлами.ПолучитьФайлСSFTP(АдресСервера,
	                                 ИмяКаталога + "/" + ТестовыйФайл.Имя,
	                                 КаталогВременныхДанных);
