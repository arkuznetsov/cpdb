...
Процедура ТестДолжен_СоздатьПапкуВNextCloud() Экспорт
	...
	Клиент.СоздатьПапкуВNextCloud(ИмяКаталога);
	...
	Утверждения.ПроверитьИстину(Клиент.Соединение().Файлы().Существует(ИмяКаталога), ТекстОшибки);

	Клиент.Соединение().Файлы().Удалить(ИмяКаталога);
...
Процедура ТестДолжен_ОтправитьФайлВNextCloud() Экспорт
	...
	Клиент.ОтправитьФайлВNextCloud(ТестовыйФайл.ПолноеИмя, ИмяКаталога);
	...
	Утверждения.ПроверитьИстину(Клиент.Соединение().Файлы().Существует(ОбъединитьПути(ИмяКаталога, ТестовыйФайл.Имя)), ТекстОшибки);

	Клиент.Соединение().Файлы().Удалить(ОбъединитьПути(ИмяКаталога, ТестовыйФайл.Имя));
...
Процедура ТестДолжен_ПолучитьФайлИзNextCloud() Экспорт
	...
	Клиент.ПолучитьФайлИзNextCloud(ОбъединитьПути(ИмяКаталога, ТестовыйФайл.Имя),
...
Процедура ТестДолжен_СоздатьПапкуНаSFTP() Экспорт
	...
	ПортСервера        = 2222;
	АдресСервера       = СтрШаблон("%1:%2", АдресСервера, ПортСервера);
	...
	Клиент.СоздатьПапкуНаSFTP(ИмяКаталога);
	...
	ФайлыНаСервере = Клиент.Соединение().ПолучитьScp();

	Утверждения.ПроверитьИстину(ФайлыНаСервере.Существует(ИмяКаталога), ТекстОшибки);

	ФайлыНаСервере.УдалитьКаталог(ИмяКаталога);
...
Процедура ТестДолжен_ОтправитьФайлНаSFTP() Экспорт
	...
	Клиент.ОтправитьФайлНаSFTP(ТестовыйФайл.ПолноеИмя, ИмяКаталога);
	...
	Утверждения.ПроверитьИстину(ФайлыНаСервере.Существует(ПутьКФайлу), ТекстОшибки);

	ФайлыНаСервере.УдалитьФайл(ПутьКФайлу);
...
Процедура ТестДолжен_ПолучитьФайлСSFTP() Экспорт
	...
	Клиент.ПолучитьФайлСSFTP(ПутьКФайлу, КаталогВременныхДанных);
