...
&Тест
Процедура ТестДолжен_ПолучитьОписаниеБазы() Экспорт

	Лог.Информация("Тест: Получение описания базы данных на MS SQL Server");
	...
	ТекстОшибки = СтрШаблон("Ошибка получения описания базы данных ""%1""", ИмяБД);

	ОписаниеБазы = РаботаССУБД.ПолучитьОписание(ИмяБД);
	Утверждения.ПроверитьРавенство(ОписаниеБазы.Имя, ИмяБД, ТекстОшибки);

	РаботаССУБД.УдалитьБазуДанных(ИмяБД);

	УдалитьФайлы(КаталогВременныхДанных);

КонецПроцедуры // ТестДолжен_ПолучитьОписаниеБазы()

&Тест
Процедура ТестДолжен_ИзменитьРежимДоступаБазы() Экспорт

	Лог.Информация("Тест: Изменение режима доступа базы данных на MS SQL Server");
	...
	РаботаССУБД.УстановитьОднопользовательскийРежимБазы(ИмяБД);

	ТекстОшибки = СтрШаблон("Ошибка установки однопользовательского режима базы данных ""%1""", ИмяБД);

	ОписаниеБазы = РаботаССУБД.ПолучитьОписание(ИмяБД);
	Утверждения.ПроверитьРавенство(ОписаниеБазы.РежимДоступа, "SINGLE_USER", ТекстОшибки);

	РаботаССУБД.УстановитьМногопользовательскийРежимБазы(ИмяБД);

	ТекстОшибки = СтрШаблон("Ошибка установки многопользовательского режима базы данных ""%1""", ИмяБД);

	ОписаниеБазы = РаботаССУБД.ПолучитьОписание(ИмяБД);
	Утверждения.ПроверитьРавенство(ОписаниеБазы.РежимДоступа, "MULTI_USER", ТекстОшибки);

	РаботаССУБД.УдалитьБазуДанных(ИмяБД);

	УдалитьФайлы(КаталогВременныхДанных);

КонецПроцедуры // ТестДолжен_ИзменитьРежимДоступаБазы()
