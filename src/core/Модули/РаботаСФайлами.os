...
// Создает папку на сервере SFTP
//
// Параметры:
//   Соединение     - КлиентSSH    - объект для работы с сервером SFTP
//   ЦелевойПуть    - Строка       - путь к создаваемому каталогу
//
Процедура СоздатьПапкуНаSFTP(Соединение, Знач ЦелевойПуть) Экспорт
...
КонецПроцедуры // СоздатьПапкуНаSFTP()
...
//   Перезаписывать   - Булево                  - перезаписать файл в сервисе NextCloud при загрузке
//
Процедура ОтправитьФайлНаSFTP(Соединение, Знач ПутьКФайлу, Знач ЦелевойПуть, Перезаписывать = Ложь) Экспорт
	...
	Попытка
		...
		ФайлыНаСервере.ОтправитьФайл(ИсходныйФайл.ПолноеИмя, ЦелевойПуть, Перезаписывать);

		Лог.Информация("Файл загружен на сервер SFTP ""%1"" -> ""%2""", ПутьКФайлу, ЦелевойПуть);
	Исключение
		ФайлыНаСервере.Закрыть();
...
КонецПроцедуры // ОтправитьФайлНаSFTP()
...
//   Строка    - путь к полученному файлу
//
Функция ПолучитьФайлСSFTP(Соединение, Знач ПутьНаСервере, Знач ЦелевойКаталог, УдалитьИсточник = Ложь) Экспорт
	
	ЧастиПути = ЧастиПути(ПутьНаСервере);

	ЦелевойПуть = ОбъединитьПути(ЦелевойКаталог, ЧастиПути.Имя);

	ЦелевойПуть = ФС.ПолныйПуть(ЦелевойПуть);

	Лог.Информация("Начало получения файла ""%1"" -> ""%2""", ПутьНаСервере, ЦелевойПуть);

	Попытка
		ФайлыНаСервере = Соединение.ПолучитьScp();
		
		ФайлыНаСервере.ПолучитьФайл(ПутьНаСервере, ЦелевойКаталог);
	...
	Если УдалитьИсточник Тогда
		ФайлыНаСервере.УдалитьФайл(ПутьНаСервере);
		Лог.Информация("Удален файл на сервере SFTP ""%1""", ПутьНаСервере);
	КонецЕсли;
...
КонецФункции // ПолучитьФайлСSFTP()
