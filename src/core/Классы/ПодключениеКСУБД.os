...
Функция ВосстановитьИзРезервнойКопии(База,
	...
	ФайлРезервнойКопии = Новый Файл(ПутьКРезервнойКопии);
	ПутьКРезервнойКопии = ФайлРезервнойКопии.ПолноеИмя;

	Если ЗначениеЗаполнено(ПутьКФайлуДанных) Тогда
		КаталогДанных = Новый Файл(ПутьКФайлуДанных);
		ПутьКФайлуДанных = КаталогДанных.ПолноеИмя;
	Иначе
		ПутьКФайлуДанных = РасположениеФайловБазПоУмолчанию("D")
	КонецЕсли;

	Если ЗначениеЗаполнено(ПутьКФайлуЖурнала) Тогда
		КаталогЖурнала = Новый Файл(ПутьКФайлуЖурнала);
		ПутьКФайлуЖурнала = КаталогЖурнала.ПолноеИмя;
	Иначе
...
//   Строка     - логическое имя файла базы
//
Функция РасположениеФайловБазПоУмолчанию(Знач ТипФайла = "ROWS") Экспорт
