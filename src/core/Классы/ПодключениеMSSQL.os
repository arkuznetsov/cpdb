...
Функция СписокБаз(ФильтрБазПоИмени = "", ФильтрПоТаблице = "", ВключаяСистемные = Ложь) Экспорт
	...
	Для й = 1 По Текст.КоличествоСтрок() Цикл
		...
		ОписаниеБазы = Новый Структура();
		ОписаниеБазы.Вставить("Имя"               , 0);
		ОписаниеБазы.Вставить("Ид"                , 1);
		ОписаниеБазы.Вставить("ИдСоздателя"       , 2);
		ОписаниеБазы.Вставить("Статус1"           , 3);
		ОписаниеБазы.Вставить("Статус2"           , 4);
		ОписаниеБазы.Вставить("ДатаСоздания"      , 5);
		ОписаниеБазы.Вставить("РежимСовместимости", 6);
		ОписаниеБазы.Вставить("ФайлДанных"        , 7);
		...
		Для Каждого ТекЭлемент Из ОписаниеБазы Цикл
			...
			Если Индекс >= МассивЗначений.Количество() Тогда
				ОписаниеБазы[Ключ] = Неопределено;
				Продолжить;
			КонецЕсли;

			ОписаниеБазы[Ключ] = СокрЛП(МассивЗначений[Индекс]);

			Если Ключ = "ДатаСоздания" Тогда
				ДатаИВремя = СтрРазделить(ОписаниеБазы[Ключ], " ", Ложь);

				ЧастиДаты = СтрРазделить(ДатаИВремя[0], "-", Ложь);
				ЧастиВремени = СтрРазделить(ДатаИВремя[1], ".", Ложь);
				ЧастиВремени = СтрРазделить(ЧастиВремени[0], ":", Ложь);

				ОписаниеБазы[Ключ] = Дата(Число(ЧастиДаты[0]), Число(ЧастиДаты[1]), Число(ЧастиДаты[2]),
				                          Число(ЧастиВремени[0]), Число(ЧастиВремени[1]), Число(ЧастиВремени[2]));
			КонецЕсли;
		КонецЦикла;
