...
//
Процедура СоздатьПапкуНаSFTP(Знач ЦелевойПуть) Экспорт
...
//
Процедура ОтправитьФайлНаSFTP(Знач ПутьКФайлу, Знач ЦелевойПуть, Перезаписывать = Ложь) Экспорт
...
//
Функция ПолучитьФайлСSFTP(Знач ПутьНаСервере, Знач ЦелевойКаталог, УдалитьИсточник = Ложь) Экспорт
	...
	Если УдалитьИсточник Тогда
		...
		Лог.Информация("Удален файл  c SFTP-сервера ""%1""", ПутьНаСервере);
	КонецЕсли;
