...
Функция ОписаниеМетаданныхОбъектовБД1С(ДобавлятьКолонки = Ложь) Экспорт
	...
	Для Каждого ТекЭлемент Из ИменаТаблицОбъектовКонфигурации1С Цикл
		...
		ОписаниеОбъектаБазы = ОписаниеОбъектаБазыПоСоответствиюИмениОбъектаИИд(ТекЭлемент.Значение, ДобавлятьКолонки);
...
//  Структура    - имя объекта базы, префикс, индекс, владелец и суффикс (см. СтруктураОписанияОбъектаБазы())
//
Функция ОписаниеОбъектаБазыПоСоответствиюИмениОбъектаИИд(СоответствиеИмениОбъектаИИд, ДобавлятьКолонки)
...
Функция ЭтоЧисло(Строка)
	...
	Для й = 1 По СтрДлина(Строка) Цикл
		ТекущийСимвол = Сред(Строка, й, 1);
		Если ТекущийСимвол = "." Тогда
			КоличествоТочек = КоличествоТочек + 1;
		ИначеЕсли СтрНайти(Цифры, ТекущийСимвол) = 0 Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
