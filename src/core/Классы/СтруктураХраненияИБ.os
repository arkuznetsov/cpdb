...
Перем РегуляркаПроверкиЧисла;      // - РегулярноеВыражение - регулярное выражение для проверки числа
Перем РегуляркаОднотипныхСимволов; // - РегулярноеВыражение - регулярное выражение для проверки числа
...
Функция ОписаниеМетаданныхОбъектовБД1С(ДобавлятьКолонки = Ложь) Экспорт
	...
	Для Каждого ТекЭлемент Из ИменаТаблицОбъектовКонфигурации1С Цикл
		...
		Если НЕ ОписанияМетаданных[Имя] = Неопределено
...
Функция ПрочитатьОднотипныеСимволы(Знач ИсходнаяСтрока)
	...
	Совпадения = РегуляркаОднотипныхСимволов.НайтиСовпадения(ИсходнаяСтрока);
...
Функция ЭтоЧисло(Строка)
	...
	Возврат РегуляркаПроверкиЧисла.Совпадает(Строка);
...
Процедура ПриСозданииОбъекта(Знач _ПодключениеКСУБД, _База)
	...
	РегуляркаПроверкиЧисла = Новый РегулярноеВыражение("^\d+$|^\d+.\d+$");
	РегуляркаОднотипныхСимволов = Новый РегулярноеВыражение("^\d+|^[a-zAZ]+");
