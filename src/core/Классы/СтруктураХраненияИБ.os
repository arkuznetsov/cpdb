...
Функция ОписаниеМетаданныхОбъектаБД1С(Знач ИмяОбъекта) Экспорт
	...
	Если НазначениеОбъектаБазы(ОписаниеОбъектаБазы.Префикс).Назначение = "Поле" Тогда
...
Функция ОписаниеМетаданныхОбъектовБД1С(ДобавлятьКолонки = Ложь) Экспорт
	...
	Для Каждого ТекЭлемент Из ИменаТаблицОбъектовКонфигурации1С Цикл
		...
		Если НазначениеОбъектаБазы(ТекЭлемент.Значение.Префикс).Назначение = "Поле" Тогда
		...
		Пока ЗначениеЗаполнено(ОписаниеВладельца.Владелец) Цикл
			...
			Если НазначениеОбъектаБазы(ОписаниеВладельца.Префикс).Подчиненный Тогда
...
Функция ИменаТаблицОбъектовКонфигурации1С(ДобавлятьКолонки = Ложь)
	...
	Пока НЕ ТекСтрока = Неопределено Цикл
		...
		Если НЕ ДобавлятьКолонки И НазначениеОбъектаБазы(СтрокаСоответствия.Префикс).Назначение = "Поле" Тогда
			ТекСтрока = Чтение.ПрочитатьСтроку();
			Продолжить;
		КонецЕсли;
...
//
Функция НазначениеОбъектаБазы(ПрефиксОбъекта)
	...
	Если НазначенияОбъектовБазы.Свойство(ПрефиксОбъекта) Тогда
		ЗаполнитьЗначенияСвойств(ОписаниеНазначения, НазначенияОбъектовБазы[ПрефиксОбъекта]);
	КонецЕсли;
...
Функция ОписаниеОбъектаБазыПоИмени(Знач ИмяОбъектаБазы)
	...
	ЗаполнитьЗначенияСвойств(ОписаниеОбъектаБазы, НазначениеОбъектаБазы(ПрефиксОбъекта));
