...
Функция ОписаниеМетаданныхОбъектаБД1С(Знач ИмяОбъекта) Экспорт
	...
	Если НазначенияОбъектовБазы.Свойство(ОписаниеОбъектаБазы.Префикс) Тогда
		НазначениеОбъекта = НазначенияОбъектовБазы[ОписаниеОбъектаБазы.Префикс].Назначение;
	КонецЕсли;

	Если НазначениеОбъекта = "Поле" Тогда
...
Функция ОписаниеМетаданныхОбъектовБД1С(ДобавлятьКолонки = Ложь) Экспорт
	...
	Для Каждого ТекЭлемент Из СоответствиеИдМетаданныхИменамТаблиц Цикл
		...
		НазначениеОбъекта = "";
		Если НазначенияОбъектовБазы.Свойство(ТекЭлемент.Значение.Префикс) Тогда
			НазначениеОбъекта = НазначенияОбъектовБазы[ТекЭлемент.Значение.Префикс].Назначение;
		КонецЕсли;
		...
		Пока ЗначениеЗаполнено(ОписаниеВладельца.Владелец) Цикл
			...
			Если НазначенияОбъектовБазы.Свойство(ОписаниеВладельца.Префикс)
			   И НазначенияОбъектовБазы[ОписаниеВладельца.Префикс].Подчиненный Тогда
...
Функция ЗанимаемоеМесто() Экспорт
	...
	Если КодВозврата = 0 Тогда
		...
		Для Каждого ТекЭлемент Из СтруктураРезультата Цикл
			...
			Размерность = ВРег(Прав(СокрЛП(МассивЗначений[Индекс]), СимволовРазмерности));
...
Функция СоответствиеИдМетаданныхИменамТаблиц(ДобавлятьКолонки = Ложь)
	...
	Пока НЕ ТекСтрока = Неопределено Цикл
		...
		НазначениеОбъекта = "";
		Если НазначенияОбъектовБазы.Свойство(СтрокаСоответствия.Префикс) Тогда
			НазначениеОбъекта = НазначенияОбъектовБазы[СтрокаСоответствия.Префикс].Назначение;
		КонецЕсли;

		Если НЕ ДобавлятьКолонки И НазначениеОбъекта = "Поле" Тогда
			ТекСтрока = Чтение.ПрочитатьСтроку();
			Продолжить;
		КонецЕсли;

		СоответствиеИменБД.Вставить(СтрШаблон("%1%2", СтрокаСоответствия.Префикс, ИндексСтрокой), СтрокаСоответствия);
...
Процедура ДополнитьСоответствиеИменМетаданных(СоответствиеИменМетаданных, ТекстXML)
	...
	Пока Парсер.Прочитать() Цикл
		...
		Если ЗначенияАтрибутов.Количество() = 0 ИЛИ Ид = Неопределено
		 ИЛИ Данные = Неопределено ИЛИ СтрДлина(Ид) <> ДлинаИд Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПрочитатьСимволы(Знач ИсходнаяСтрока, ЧитатьЧисла = Ложь)
	...
	Пока НЕ (ЭтоЧисло(Символ) ИЛИ ЧитатьЧисла)
	 ИЛИ (ЭтоЧисло(Символ) И ЧитатьЧисла) Цикл
...
Функция ОписаниеОбъектаБазыПоИмени(Знач ИмяОбъектаБазы)
	...
	ПрефиксОбъекта = ПрочитатьСимволы(ИмяОбъектаБазы);
	...
	Если НазначенияОбъектовБазы.Свойство(ПрефиксОбъекта) Тогда
		НазначениеОбъекта = НазначенияОбъектовБазы[ПрефиксОбъекта];
	...
	ИндексОбъекта = ПрочитатьСимволы(ИмяОбъектаБазы, Истина);
...
Функция КолонкиБазыДанных(ФильтрТаблицПоИмени = "", ФильтрКолонокПоИмени = "")
	...
	Лог.Отладка("Получение списка колонок базы ""%1\%2""", ТекущийСервер, База);
	...
	Попытка
		КодВозврата = ПодключениеКСУБД.ВыполнитьЗапросСУБД(ТекстЗапроса, РезультатЗапроса, "|", Истина);
		Если КодВозврата = 0 Тогда
			Лог.Отладка("Получен список колонок базы ""%1\%2""",
			            ТекущийСервер,
			            База);
		Иначе
