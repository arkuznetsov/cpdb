...
Процедура СоздатьБазуДанных(База, МодельВосстановления = "FULL", ПутьККаталогу = "") Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.СоздатьБазу(База, МодельВосстановления, ПутьККаталогу, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура УдалитьБазуДанных(База) Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.УдалитьБазу(База, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ВыполнитьРезервноеКопирование(База, ПутьКРезервнойКопии) Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.СоздатьРезервнуюКопию(База, ПутьКРезервнойКопии, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ИзменитьВладельца(База, ВладелецБазы) Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.УстановитьВладельцаБазы(База, ВладелецБазы, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ИзменитьМодельВосстановления(База, МодельВосстановления) Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.УстановитьМодельВосстановления(База, МодельВосстановления, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ИзменитьЛогическоеИмяФайла(База, Имя, НовоеИмя) Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.ИзменитьЛогическоеИмяФайлаВБазе(База, Имя, НовоеИмя, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ВключитьКомпрессию(База) Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.ВключитьКомпрессиюСтраниц(База, Истина, Истина, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если НЕ Результат Тогда
		ТекстОшибки = СтрШаблон("Ошибка включения компрессии страниц в базе ""%1"": %2%3",
		                        База,
		                        Символы.ПС,
		                        РезультатЗапроса);
		ВызватьИсключение ТекстОшибки;
...
Процедура СжатьБазу(База) Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.СжатьБазу(База, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если НЕ Результат Тогда
		ТекстОшибки = СтрШаблон("Ошибка сжатия файла данных базы ""%1"": %2%3", База, Символы.ПС, РезультатЗапроса);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;

	Лог.Отладка("Выполнено сжатие файла данных базы ""%1""", База);
...
Процедура СжатьФайлЖурналаТранзакций(База) Экспорт
	...
	Попытка
		Результат = ПодключениеКСУБД.СжатьФайлЖурналаТранзакций(База, РезультатЗапроса);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Лог.Отладка("Выполнено сжатие файла журнала транзакций базы ""%1""", База);
