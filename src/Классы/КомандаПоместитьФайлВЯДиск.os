...
Функция ОтправитьФайлНаЯДиск(ЯДиск = Неопределено, Знач Каталог, Знач ИмяФайла, Знач ЦелевойПуть)
	...
	ЯДиск.ЗагрузитьНаДиск(ИсходныйФайл.ПолноеИмя, ЦелевойПуть + "\" + ИсходныйФайл.Имя);
	Попытка
		СвойстваФайла = ЯДиск.ПолучитьСвойстваРесурса(ЦелевойПуть + "\" + ИсходныйФайл.Имя);
	Исключение
		Лог.Ошибка("Ошибка при получении свойств ресурса: %1", ИнформацияОбОшибке());
	КонецПопытки;
	
	Лог.Информация("Файл загружен %1", СвойстваФайла.name);
