...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Парсер.ДобавитьПараметрФлагКоманды(ОписаниеКоманды, 
		"-shrink-log",
		"Сжать файл лога после выполнения компрессии");
...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды) Экспорт
	...
	СжатьФайлЛог			= ПараметрыКоманды["-shrink-log"];
...
//	Булево - Истина - команда выполнена успешно; Ложь - в противном случае
//
Функция СжатьФайлЛог(База) Экспорт

	Лог.Информация("Начало сжатия (shrink) файла ""%1_log""", База);
		
	Попытка
		Результат = Инструменты.СжатьФайлЛог(База);

		Если НЕ Результат Тогда
			Лог.Ошибка("Ошибка сжатия файла ""%1_log""", База);
			Возврат Ложь;
		КонецЕсли;

		Лог.Информация("Выполнено сжатие файла ""%1_log""", База);
	Исключение
		Лог.Ошибка("Ошибка сжатия файла ""%1_log"": ""%2""", База, ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
