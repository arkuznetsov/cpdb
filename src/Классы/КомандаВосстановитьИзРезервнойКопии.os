...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды) Экспорт
	...
	Попытка
		...
		Если Результат = 0 Тогда
			Лог.Ошибка("Ошибка восстановления из резервной копии: " + ОписаниеРезультата);
			Возврат ВозможныйРезультат.ОшибкаВремениВыполнения;
		КонецЕсли;
	...
	Исключение
		Лог.Ошибка("Ошибка восстановления из резервной копии: " + ОписаниеОшибки() +
				   ?(ЗначениеЗаполнено(ОписаниеРезультата), Символы.ПС + ОписаниеРезультата, ""));
		Возврат ВозможныйРезультат.ОшибкаВремениВыполнения;
	КонецПопытки;
	...
	Если ЗначениеЗаполнено(ВладелецБазы) Тогда
		Попытка
			Инструменты.УстановитьВладельцаБазы(База, ВладелецБазы);
	...
	Если ЗначениеЗаполнено(МодельВосстановления) Тогда
		Попытка
			Инструменты.УстановитьМодельВосстановления(База, МодельВосстановления);
	...
	Если ИзменитьЛИФ Тогда
		Попытка
			...
			НовоеЛИФ = База;
			Инструменты.ИзменитьЛогическоеИмяФайлаБазы(База, ЛИФ, НовоеЛИФ);
			Лог.Информация("Для базы ""%1"" изменено логическое имя файла данных ""%2"" на ""%3""", База, ЛИФ, НовоеЛИФ);
		Исключение
			Лог.Ошибка("Ошибка изменения логического имени файла данных ""%1"" в базе ""%2"": %3", ЛИФ, База);
			Возврат ВозможныйРезультат.ОшибкаВремениВыполнения;
		КонецПопытки;
		Попытка
